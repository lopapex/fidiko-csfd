...
function FilmMini({ film }: { film: FilmGroup }) {
  return (
    <div className="weekly-film-summary">
      ...
      <div className="weekly-film-copy"><strong>{film.title}</strong><div className="weekly-film-meta">{film.csfd?.rating != null ? film.csfd.url ? <a className={`weekly-rating ${getRatingClass(film.csfd.rating)}`} href={film.csfd.url} target="_blank" rel="noopener noreferrer" aria-label={`${film.title} na ČSFD, hodnocení ${film.csfd.rating} %`}>{film.csfd.rating}%</a> : <span className={`weekly-rating ${getRatingClass(film.csfd.rating)}`}>{film.csfd.rating}%</span> : null}{film.hasSubtitles ? <span className="weekly-subtitle-mark">Titulky</span> : null}</div></div>
    </div>
  );
}
    ...
    <article className={film.hasSubtitles ? "film-row film-row-subtitles" : "film-row"} id={film.id}>
      <div className="film-info">
        ...
        <div className="film-copy">
          ...
          <div className="csfd-block"><div className="csfd-line">{film.csfd?.rating != null ? film.csfd.url ? <a className={`rating-badge rating-link ${getRatingClass(film.csfd.rating)}`} href={film.csfd.url} target="_blank" rel="noopener noreferrer" aria-label={`${film.title} na ČSFD, hodnocení ${film.csfd.rating} %`}>{film.csfd.rating}%</a> : <span className={`rating-badge ${getRatingClass(film.csfd.rating)}`}>{film.csfd.rating}%</span> : <span className="rating-badge rating-missing">?</span>}<span className="rating-copy">{getCsfdStatusText(film.csfd)}</span>{film.csfd?.url ? <a className="csfd-button csfd-button-desktop" href={film.csfd.url} target="_blank" rel="noopener noreferrer">ČSFD</a> : null}</div></div>
        </div>
      </div>
